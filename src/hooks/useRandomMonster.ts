...
export function useRandomMonster (): MonsterVisualProfile | null {
  ...
  useEffect(() => {
    ...
    const generateRandomMonster = async (): Promise<void> => {
      ...
      if (result.success) {
        setMonsterProfile(result.data)
      } else {
        console.error('Erreur lors de la génération du monstre:', result.error)
      }
    }
